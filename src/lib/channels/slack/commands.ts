/**
 * Parse PaulAgentBot commands from Slack message text.
 *
 * Supported forms (when bot is mentioned):
 *   @paulagentbot repo owner/name
 *   @paulagentbot status
 *   @paulagentbot new
 *   @paulagentbot stop
 *
...
export type PaulAgentBotCommand =
 ...
 */
export function parseCommand(text: string): PaulAgentBotCommand | null {
  ...
  const slashMatch = clean.match(/^\/paulagentbot\s+(\w+)(?:\s+(.+))?$/i)
...
/**
 * Returns true if the text appears to be a PaulAgentBot command (not a free-form prompt).
 */
